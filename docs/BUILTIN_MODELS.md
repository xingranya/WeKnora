...
若模型已经通过管理界面完成连接配置，不应把现有凭据复制到 YAML。可在同一文件中按稳定 ID 声明采用清单：

```yaml
adopt_existing_model_ids:
...
启动加载器会查找这些现有模型并仅把 `is_builtin` 更新为 `true`。模型的 `tenant_id`、`managed_by`、默认状态、参数、扩展配置和加密凭据保持不变。采用完成后，模型通过现有查询条件对所有空间可见，普通用户收到的响应仍执行内置模型脱敏。

采用清单具有以下边界：

- ID 重复时只处理一次，重复启动保持幂等。
- ID 不存在或格式无效时记录警告，不创建缺少连接参数的空模型。
- 这是单向提升。把 ID 从清单移除不会自动降级或删除模型，避免配置文件短暂缺失导致公司模型消失。
- 如需取消公司预置标记，由系统管理员明确执行 `UPDATE models SET is_builtin = false ...`。
