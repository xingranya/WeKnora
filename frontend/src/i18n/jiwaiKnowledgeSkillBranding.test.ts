...
test('见外浏览器插件使用公司品牌版本和独立下载地址', () => {
  assert.equal(CHROME_EXTENSION_DOWNLOAD_URL, '/downloads/jiwai-knowledge-assistant-1.3.0.zip')
  for (const [localeName, locale] of Object.entries(localeBundles)) {
    assert.match(locale.integrations.chrome.storeMeta, /v1\.3\.0/, `${localeName} 插件版本不是 v1.3.0`)
  }
