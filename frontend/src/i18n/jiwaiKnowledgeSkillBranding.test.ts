...
test('见外知识库展示名在所有主界面语言中保持一致', () => {
  for (const [localeName, locale] of Object.entries(localeBundles)) {
    ...
    assert.match(locale.integrations.claw.downloadCtaHint, /v1\.5\.0/, `${localeName} 下载版本不是 v1.5.0`)
    assert.match(locale.integrations.claw.packageMeta, /v1\.5\.0/, `${localeName} 包版本不是 v1.5.0`)
  }
})
