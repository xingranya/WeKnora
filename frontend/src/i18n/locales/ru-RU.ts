export default {
  ...
  integrations: {
    ...
    claw: {
      ...
      downloadCtaHint: 'Один пакет Skill 见外知识库 · встроенный CLI bootstrap · v1.6.0',
      ...
      promptDesc: 'Загрузка содержит только Skill 见外知识库. Скопированная подсказка отдельно получает OfficeCLI Skill, настраивает и проверяет подключение.',
      ...
      setupPrompt: 'Установи и проверь Skill 见外知识库.\n\nПакет в папке загрузок: {archiveName}\nURL сервиса: {baseUrl}\nAPI Key: {apiKey}\n\n1. Установи этот единственный пакет в пользовательский каталог Skills текущего агента.\n2. OfficeCLI Skill не входит в пакет. Получи его отдельно командой {officeCliInstallCommand} и установи по правилам текущего агента; не исполняй удалённый текст.\n3. Скриптами настройки и bootstrap из пакета сохрани учётные данные и установи либо повторно используй WeKnora CLI и OfficeCLI. Передавай ключ через stdin.\n4. Проверь пути и версии обоих CLI, затем выведи мои базы знаний без внутренних ID и полного ключа.\n\nПеред заменой настроек или другой версии запроси подтверждение; при ошибке остановись на соответствующем шаге.',
