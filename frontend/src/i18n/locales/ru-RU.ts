export default {
  ...
  integrations: {
    ...
    claw: {
      title: '见外知识库',
      subtitle: 'Скачайте универсальный пакет Skill, выберите существующий API Key пространства и поручите любому агенту автоматическую установку и подключение.',
      compatibility: {
        codex: 'Codex',
        claude: 'Claude Code',
        cursor: 'Cursor',
        openclaw: 'OpenClaw',
        generic: 'Другие платформы Agent Skills'
      },
      downloadCta: 'Скачать ZIP 见外知识库',
      downloadCtaHint: 'Скрипты настройки пользовательской среды macOS и Windows · v1.2.1',
      apiKeyTitle: 'Выберите API Key',
      apiKeyDesc: 'Выберите ключ, уже созданный в текущем пространстве. В списке значение скрыто, а скопированная подсказка содержит полный ключ.',
      apiKeyPlaceholder: 'Выберите API Key',
      apiKeyPromptPlaceholder: '<заполнится после выбора API Key>',
      apiKeyFullAccess: 'Полный доступ к пространству',
      apiKeyScopedAccess: 'Ограниченный доступ',
      expiredKeysIgnored: 'Просроченные ключи исключены автоматически: {count}.',
      noApiKeys: 'В пространстве нет пригодных API Key или все ключи просрочены. Сначала создайте ключ.',
      createApiKey: 'Создать API Key',
      retry: 'Повторить',
      openApiSettings: 'Открыть интеграцию API',
      loadApiKeysFailed: 'Не удалось загрузить API Key',
      tenantUnavailable: 'Текущее пространство пока недоступно. Обновите страницу и повторите попытку.',
      selectApiKeyFirst: 'Сначала выберите API Key',
      promptTitle: 'Автоматическая установка через ИИ',
      promptDesc: 'Универсальная подсказка просит текущий ИИ определить платформу агента и ОС, установить Skill, настроить переменные среды и проверить соединение.',
      copyPrompt: 'Скопировать для ИИ',
      copyPromptSuccess: 'Подсказка установки скопирована',
      securityNote: 'В предпросмотре полный ключ всегда скрыт; скопированный текст содержит учётные данные. Отправляйте его только доверенному ИИ и используйте минимально необходимые права.',
      setupPrompt: 'Установи и настрой Skill “见外知识库” в текущем агенте.\n\nПакет: найди “{archiveName}” в стандартной папке загрузок системы.\nВыполни следующее:\n1. Определи пользовательский каталог Skill текущего агента и распакуй пакет туда. Если платформа поддерживает стандарт Agent Skills, используй её стандартный каталог.\n2. Определи macOS или Windows и запусти соответствующий скрипт настройки из пакета. Сохрани следующие переменные в пользовательской среде, а не только во временной текущей оболочке:\nWEKNORA_BASE_URL="{baseUrl}"\nWEKNORA_API_KEY="{apiKey}"\n3. Перезагрузи среду, затем выведи список баз знаний через Skill и проверь соединение.\n\nБезопасность: не показывай полный API Key в выводе команд, файлах проекта или финальном ответе. Перед перезаписью настроек сообщи, какой путь будет изменён.',
      stepsTitle: 'Процесс установки',
      steps: {
        download: {
          title: 'Скачайте пакет',
          desc: 'Скачайте полный ZIP кнопкой выше. Не нужно вручную распаковывать его или искать каталог установки.'
        },
        key: {
          title: 'Выберите существующий ключ',
          desc: 'Выберите один из API Key текущего пространства; его область разрешений сохранится.'
        },
        prompt: {
          title: 'Передайте текущему ИИ',
          desc: 'ИИ определит Codex, Claude Code, Cursor, OpenClaw или другую платформу, а также macOS или Windows.'
        },
        verify: {
          title: 'Проверьте соединение',
          desc: 'ИИ перезагрузит среду и выведет список баз знаний, проверив установку, учётные данные и сеть.'
        }
      },
      capabilitiesTitle: 'Доступно после установки',
      capabilities: {
        browse: {
      ...
      ecosystemNote: 'Пакет следует правилам каталогов Agent Skills; версия для OpenClaw также доступна через ClawHub.',
    ...
    chrome: {
      ...
      steps: {
        ...
        install: {
          ...
          desc: 'Скачайте и распакуйте ZIP-архив, откройте chrome://extensions или edge://extensions, включите режим разработчика, выберите «Загрузить распакованное расширение» и укажите папку с manifest.json.'
        },
        port: {
  ...
  modelSettings: {
    ...
    copySuffix: ' копия',
    builtinTag: 'Предустановлено компанией',
    ...
    builtinModels: {
      title: 'Предустановленные модели компании',
      description: 'Предустановленные модели компании видны всем пространствам. Сотрудники не могут изменять, удалять или тестировать их, а конфиденциальные данные скрыты.',
      descriptionAdmin: 'Предустановленные модели компании видны всем пространствам. Системные администраторы могут изменять конфигурацию и учетные данные; удаление управляется конфигурацией развертывания.',
      viewGuide: 'Руководство по предустановленным моделям компании'
    },
    toasts: {
      ...
      builtinCannotEdit: 'Предустановленные модели компании нельзя редактировать',
      builtinCannotDelete: 'Предустановленные модели компании нельзя удалить',
      builtinCannotCopy: 'Предустановленные модели компании нельзя копировать',
  ...
  chat: {
    ...
    historyLoadFailed: 'Не удалось загрузить историю диалога. Проверьте сеть и повторите попытку.',
    retryHistoryLoad: 'Перезагрузить',
    historyLoadMoreFailed: 'Не удалось загрузить более ранние сообщения.',
    retryHistoryLoadMore: 'Повторить',
  ...
  settings: {
    ...
    parser: {
      title: 'Парсер',
      description: 'Парсеры документов централизованно управляются компанией. Сотрудники могут сразу использовать их, а параметры подключения видны только системным администраторам.',
      companyPreset: 'Предустановлено компанией',
      ...
      maxConcurrency: 'Максимальный параллелизм',
      maxConcurrencyHint: 'Значение должно совпадать с MinerU; лишние задачи ожидают в WeKnora.',
  ...
  uploadConfirm: {
    ...
    parserFileSizeExceeded: 'Файл «{file}» превышает лимит {size} МБ для анализатора {engine}. Выберите другой анализатор или разделите файл.',
    urlItemLabel: 'URL',
  ...
  knowledgeBase: {
    ...
    folderTree: {
      ...
      moreOptions: 'Другие действия с папкой',
      ...
      create: 'Новая папка',
      createChild: 'Новая вложенная папка',
      createPlaceholder: 'Название папки',
      createSuccess: 'Папка создана',
      createFailed: 'Не удалось создать папку',
      delete: 'Удалить папку',
      deleteSuccess: 'Папка удалена',
      deleteFailed: 'Сначала переместите или удалите документы из папки',
    },
    moveToFolder: {
    ...
    uploadQueue: {
      title: 'Очередь загрузки', summary: 'Незавершённых задач: {count}', empty: 'Нет загрузок',
      pause: 'Пауза', retry: 'Продолжить или повторить', cancel: 'Отменить', remove: 'Удалить', selectFile: 'Выберите исходный файл для продолжения',
      progress: 'Загрузка файла «{file}»: {percent}%',
      secondsLeft: 'около {count} с', minutesLeft: 'около {count} мин', hoursLeft: 'около {count} ч',
      status: { queued: 'В очереди', uploading: 'Загрузка', paused: 'Приостановлено', needs_file: 'Нужен исходный файл', completing: 'Объединение', waiting_parse: 'Ожидание анализа', parsing: 'Анализ', status_unknown: 'Статус уточняется', completed: 'Готово', failed: 'Ошибка', cancel_requested: 'Отмена', cancelled: 'Отменено' },
      errors: { uploadFailed: 'Не удалось загрузить файл', parseTimeout: 'Время ожидания статуса анализа истекло', statusCheckFailed: 'Последний статус временно недоступен. Автоматическая проверка продолжится.', parseTakingLonger: 'Обработка занимает больше времени, чем ожидалось. Автоматическая проверка продолжится.', fileMismatch: 'Выбранный файл не совпадает с исходным', parseRetryFailed: 'Не удалось повторно запустить анализ', cancelFailed: 'Не удалось отменить задачу', cancelStatusUnknown: 'Результат отмены временно недоступен. Автоматическая проверка продолжится.', sessionExpired: 'Сеанс загрузки истёк. Загрузите файл повторно.' },
    },
    ...
    documentLoadFailed: 'Не удалось загрузить документы. Проверьте сеть и повторите попытку.',
    retryLoadDocuments: 'Перезагрузить',
    missingId: 'Отсутствует ID базы знаний',
    ...
    pdfDocFormat: 'Поддерживаются PDF, Office, изображения, аудио и другие форматы',
    textMarkdownFormat: 'До 2 ГБ на файл с автоматической постановкой в очередь разбора',
