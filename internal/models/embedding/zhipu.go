...
func (e *ZhipuEmbedder) BatchEmbed(ctx context.Context, texts []string) ([][]float32, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		logger.GetLogger(ctx).Errorf("ZhipuEmbedder BatchEmbed API error: Http Status %s, response=%q",
			resp.Status, logger.AuditText(string(body), 4096))
		return nil, fmt.Errorf("BatchEmbed API error: Http Status %s, response_bytes=%d", resp.Status, len(body))
	}
